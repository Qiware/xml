...
xml_tree_t *xml_creat(const char *fname, xml_opt_t *opt)
{
    ...
    if (NULL == buff) {
        fprintf(stderr, "Load xml file into memory Fail![%s]", fname);
        return NULL;
    }
...
xml_tree_t *xml_screat(const char *str, size_t len, xml_opt_t *opt)
{
    ...
    do {
        ...
        if (stack_init(&stack, XML_MAX_DEPTH)) {
            fprintf(stderr, "Init xml stack Fail!");
            break;
        }
        ...
        xml = xml_init(opt);
        if (NULL == xml) {
            fprintf(stderr, "Init xml tree Fail!");
            break;
        }

        /* 3. 解析XML文件缓存 */
        if (xml_parse(xml, &stack, str, len)) {
            fprintf(stderr, "Parse xml Fail!");
            xml_destroy(xml);
            break;
        }
...
int xml_node_free(xml_tree_t *xml, xml_node_t *node)
{
    ...
    if (stack_init(stack, XML_MAX_DEPTH)) {
        fprintf(stderr, "Init stack Fail!");
    ...
    do {
        ...
        if (stack_push(stack, curr)) {
            stack_destroy(stack);
            fprintf(stderr, "Push stack Fail!");
            return XML_ERR_STACK;
        }
...
int xml_fprint(xml_tree_t *xml, FILE *fp)
{
    ...
    while (NULL != child) {
        if (xml_fprint_tree(xml, child, &stack, fp)) {
            fprintf(stderr, "fPrint tree Fail!");
            stack_destroy(&stack);
            return XML_ERR;
        }
        child = child->next;
...
int xml_fwrite(xml_tree_t *xml, const char *fname)
{
    ...
    fp = fopen(fname, "wb");
    if (NULL == fp) {
        fprintf(stderr, "errmsg:[%d] %s! fname:%s", errno, strerror(errno), fname);
...
int xml_sprint(xml_tree_t *xml, char *str)
{
    ...
    while (NULL != child) {
        if (xml_sprint_tree(xml, child, &stack, &sp)) {
            fprintf(stderr, "Sprint tree Fail!");
            stack_destroy(&stack);
            return XML_ERR;
        }
        child = child->next;
        ...
        xml_node_t *node, const char *name, const char *value)
{
    ...
    fprintf(stderr, "Add attr node Fail!");
    return NULL;
}
...
xml_node_t *xml_add_child(xml_tree_t *xml, xml_node_t *node, const char *name, const char *value)
{
    ...
    if (NULL == child) {
        fprintf(stderr, "Create node Fail![%s]", name);
        return NULL;
    }
...
int xml_node_len(xml_tree_t *xml, xml_node_t *node)
{
    ...
    if (len < 0) {
        fprintf(stderr, "Get the len of node Fail!");
        stack_destroy(&stack);
        return -1;
    }
...
int _xml_pack_len(xml_tree_t *xml, xml_node_t *node)
{
    ...
    switch(node->type)
    {
        case XML_NODE_CHILD: /* 处理孩子结点 */
        {
            ...
            if (len < 0) {
                fprintf(stderr, "Get len of the node Fail!");
                stack_destroy(&stack);
                return -1;
            }
            break;
